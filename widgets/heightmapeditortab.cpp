...
HeightmapEditorTab::HeightmapEditorTab(QWidget* parent) : QWidget{parent} {
    ...
    graph_model_ =
        std::make_shared<QtNodes::DataFlowGraphModel>(model_registry_);
    node_scene_ =
        std::make_shared<QtNodes::DataFlowGraphicsScene>(*graph_model_);
    node_view_ = std::make_shared<QtNodes::GraphicsView>(node_scene_.get());

    l->addWidget(node_view_.get());

    QObject::connect(
        node_scene_.get(), &QtNodes::DataFlowGraphicsScene::sceneLoaded,
        node_view_.get(), &QtNodes::GraphicsView::centerScene
    );

    connect(
        graph_model_.get(), &QtNodes::DataFlowGraphModel::inPortDataWasSet,
...
std::shared_ptr<QPixmap> HeightmapEditorTab::GetHeightmap() const {
    ...
    QVariant vNoise = graph_model_->portData(
        output_node_id_, QtNodes::PortType::Out, QtNodes::PortIndex(0),
    ...
    if (noise_data != nullptr) {
        heightmap =
            std::make_shared<QPixmap>(*noise_data->AsBitmap(0, 0, 256, 256));
    }
